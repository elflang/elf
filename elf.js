var setup = function () {
  ...
  setenv("let", {_stash: true, macro: function (bs) {
    ...
    if (toplevel63()) {
      add(environment42, {});
      var _x89 = macroexpand(join(["let", bs], body));
      drop(environment42);
      return(_x89);
    }
    if (!( typeof(bs) === "object")) {
  ...
  setenv("each", {_stash: true, macro: function (x, lst) {
    ...
    if (target42 === "lua") {
      ...
      _e25 = [join(["let", k, ["if", ["numeric?", k], ["parseInt", k], k]], body)];
    }
    return(["let", [l, lst, k, "nil"], ["%for", l, k, join(["let", [v, ["get", l, k]]], _e25)]]);
  }});
...
setenv = function (k) {
  ...
  if (typeof(k) === "string") {
    var _e45;
    if (_keys.toplevel) {
      _e45 = environment42[0];
    } else {
      _e45 = last(environment42);
    }
    var frame = _e45;
    var entry = frame[k] || {};
    var _l18 = _keys;
