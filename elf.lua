local function setup()
  ...
  setenv("w/bindings", {_stash = true, macro = function (_x185, ...)
    local names = _x185[1]
    local _r46 = unstash({...})
  ...
  setenv("w/sym", {_stash = true, macro = function (expansions, ...)
    ...
    if not( type(expansions) == "table") then
      ...
      local _x208 = join({"do"}, macroexpand(body))
      drop(environment42)
      return(_x208)
    end
  end})
